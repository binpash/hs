...
Output layout, per test:
    ...
    output/<test>/strace_log, fstrace_{trace,deps,missed}
...
def hs_command(hs_base: Path, test_base: Path, script_name: str, script_args: list,
    ...
    # the internal-tooling stream -- try, fstrace, Python tracebacks -- stays
...
def fstrace_command(test_base: Path, script_name: str, script_args: list,
                    output_base: Path) -> list:
    return ['fstrace',
            '--trace-file', artifact(output_base, 'fstrace', 'trace'),
            '--dep-file', artifact(output_base, 'fstrace', 'deps'),
            '--missed-file', artifact(output_base, 'fstrace', 'missed'),
            '--', '/bin/sh', test_base / script_name] + script_args
