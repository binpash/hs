...
unset stdout_file
set +u
pash_redir_output echo "$$: [SPEC] Before restore for declare - IFS=$(declare -p IFS 2>&1 || echo 'unset'), PASH_OLD_IFS=$(declare -p PASH_OLD_IFS 2>&1 || echo 'unset')"
if [ -z "${PASH_OLD_IFS+x}" ]; then
    unset IFS
else
    IFS="$PASH_OLD_IFS"
fi
pash_redir_output echo "$$: [SPEC] After restore, before pash_declare_vars - IFS=$(declare -p IFS 2>&1 || echo 'unset')"
source "$RUNTIME_DIR/pash_declare_vars.sh" "$pash_runtime_shell_variables_file"
pash_redir_output echo "$$: [SPEC] After pash_declare_vars - IFS=$(declare -p IFS 2>&1 || echo 'unset')"
IFS=$' \t\n'
pash_redir_output echo "$$: [SPEC] After setting default - IFS=$(declare -p IFS 2>&1 || echo 'unset')"
pash_redir_output echo "$$: (1) Bash variables saved in: $pash_runtime_shell_variables_file"
pash_redir_output echo "$$: [SPEC] Contents of pash_runtime_shell_variables_file IFS/PASH_OLD_IFS lines:"
...
if [[ "$daemon_response" == *"OK:"* ]]; then
    ...
    pash_redir_output echo "$$: [SPEC] Contents of output_variable_file IFS/PASH_OLD_IFS lines:"
    pash_redir_output grep -E "^declare.*IFS" "$output_variable_file" || pash_redir_output echo "  (no IFS declarations found)"
    pash_redir_output echo "$$: [SPEC] Before pash_restore_fds - IFS=$(declare -p IFS 2>&1 || echo 'unset'), PASH_OLD_IFS=$(declare -p PASH_OLD_IFS 2>&1 || echo 'unset')"
    source "$RUNTIME_DIR/pash_restore_fds.sh" "${output_variable_file}.fds" "${stdout_file}"
    pash_redir_output echo "$$: [SPEC] After pash_restore_fds - IFS=$(declare -p IFS 2>&1 || echo 'unset'), PASH_OLD_IFS=$(declare -p PASH_OLD_IFS 2>&1 || echo 'unset')"
    source "$RUNTIME_DIR/pash_source_declare_vars.sh" "$output_variable_file"
    pash_redir_output echo "$$: [SPEC] After sourcing output_variable_file: IFS=$(declare -p IFS 2>&1 || echo 'unset'), PASH_OLD_IFS=$(declare -p PASH_OLD_IFS 2>&1 || echo 'unset')"
    ...
    pash_redir_output echo "$$: [SPEC] UNSAFE: Before restore for eval - IFS=$(declare -p IFS 2>&1 || echo 'unset'), PASH_OLD_IFS=$(declare -p PASH_OLD_IFS 2>&1 || echo 'unset')"
    if [ -z "${PASH_OLD_IFS+x}" ]; then
	unset IFS
    else
	IFS="$PASH_OLD_IFS"
    fi
    pash_redir_output echo "$$: [SPEC] UNSAFE: After restore for eval - IFS=$(declare -p IFS 2>&1 || echo 'unset')"
...
pash_redir_output echo "$$: [SPEC] End of speculative_runtime.sh - IFS=$(declare -p IFS 2>&1 || echo 'unset'), PASH_OLD_IFS=$(declare -p PASH_OLD_IFS 2>&1 || echo 'unset')"
