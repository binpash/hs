...
class SchedulingStateSet:
    ...
    def plot(self):
        ...
        fig1.update_layout(showlegend=True, xaxis_tickformat='%M:%S,%L',
                           yaxis_title="Command ID", xaxis_title="Time (ms)")
        ...
        y = [mark["y"]
             for mark in self.marks if mark["event"] == CommandState.EXECUTING]
        x = [mark["x"]
             for mark in self.marks if mark["event"] == CommandState.EXECUTING]
        fig1.add_scatter(y=y, x=x,
        ...
        # Add sandbox execution markers
        y = [mark["y"] for mark in self.marks if mark["event"]
             == CommandState.EXECUTING_SANDBOXED]
        x = [mark["x"] for mark in self.marks if mark["event"]
             == CommandState.EXECUTING_SANDBOXED]
        fig1.add_scatter(y=y, x=x,
                         marker_symbol="circle",
    ...
    def add_task(self, node_id, start, end, state):
        self.cmd_states.append(
            dict(Command_Id=str(node_id), Start=start, Finish=end, State=state.value))

    def handle_executing_add(self, object):
        node_id = int(object.message)
        self.unresolved_states[node_id] = (
            object.timestamp, CommandState.EXECUTING)
        self.marks.append(dict(y=node_id, x=object.timestamp,
                          event=CommandState.EXECUTING))

    def handle_executing_sandbox_add(self, object):
        node_id = int(object.message)
        self.unresolved_states[node_id] = (
            object.timestamp, CommandState.EXECUTING_SANDBOXED)
        self.marks.append(dict(y=node_id, x=object.timestamp,
                          event=CommandState.EXECUTING_SANDBOXED))
    ...
    def handle_frontier_add(self, object):
        node_id = int(object.message)
        self.unresolved_states[node_id] = (
            object.timestamp, CommandState.FRONTIER)
    ...
    def handle_stopped_add(self, object):
        ...
        if reason == "error":
            self.unresolved_states[node_id] = (
                object.timestamp, CommandState.STOPPED_ERROR)
        elif reason == "network":
            self.unresolved_states[node_id] = (
                object.timestamp, CommandState.STOPPED_NETWORK)
        else:
            assert False
    ...
    def handle_waiting_add(self, object):
        node_id = int(object.message)
        self.unresolved_states[node_id] = (
            object.timestamp, CommandState.WAITING)
...
def adjust_timestamp(state_set: SchedulingStateSet, trace_object):
    ...
    print(datetime(1900, 1, 1, 0, 0, 0) + (datetime.combine(date.min,
          trace_object.timestamp) - datetime.combine(date.min, t)))
    trace_object.timestamp = datetime(1900, 1, 1, 0, 0, 0) + (datetime.combine(
        date.min, trace_object.timestamp) - datetime.combine(date.min, t))


def parse_trace_objects(trace_file):
    with open(trace_file) as logfile:
        lines = logfile.read().split("\n")
    lines = [tuple(line.split("|")[1:])
             for line in lines if line.startswith("TRACE|")]
    trace_objects = [PashSpecTraceObject(parser.parse(
        timestamp), action, message) for timestamp, action, message in lines]
...
def main():
    ...
    for object in lines:
        ...
        if action == "ExecutingRemove":
            ...
            assert False, f"Not implemented handle for action: {action}"
    states.plot()
