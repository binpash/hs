...
def async_run_and_trace_command_return_trace(command, node_id, new_env_file, speculate_mode=False):
...
def async_run_and_trace_command_return_trace_in_sandbox(command, trace_file, node_id, stdout_file, stderr_file, variable_file, new_env_file, riker_env_file, speculate_mode=False):
    ...
    logging.critical(f'env__: {new_env_file}')
    if new_env_file is None:
        args.append("None")
    else:
        args.append(new_env_file)
