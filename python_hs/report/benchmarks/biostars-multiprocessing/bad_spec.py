...
This version intentionally REMOVES the prevent_java_conflicts flags to demonstrate
poor speculative execution performance due to JVM filesystem conflicts.
"""
...
for currentFile in os.listdir(Orig_file_dir):
    if currentFile.endswith(".fa"):
        ...
        perf_dir = f"/tmp/hsperfdata_{os.path.splitext(currentFile)[0]}"
        MACSE_command = f"java -jar tools/macse_v1.01b.jar -prog alignSequences -seq {input_file} -out_NT {NT_output_file} -out_AA {AA_output_file}"
